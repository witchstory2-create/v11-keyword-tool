...
def run_batch(news_list):
    ...
    for news in news_list:
        ...
        print(f"이벤트 : {result['analysis']['event']}")
        print(f"질문 : {result['analysis']['questions']}")
        ...
        results.append({"run_id": news["id"], "title": news["title"], "source": news["source"]})
    return results


def build_log_rows(batch_results):
    return [{
        "Run #": item["run_id"], "뉴스 제목": item["title"], "출처": item["source"],
        "경쟁등급(0-2/3-7/8+)": "", "애매함(Y/N)": "", "수명(즉시/단기/시즌/에버그린)": "",
        "수익(Low/Medium/High)": "", "판단(Write/Skip/Test)": "", "망설임 포인트": "",
    } for item in batch_results]


def save_log_csv(rows, filepath="experiment_log_v1.csv"):
    if HAS_PANDAS:
        pd.DataFrame(rows, columns=LOG_COLUMNS).to_csv(filepath, index=False, encoding="utf-8-sig")
    else:
        import csv
