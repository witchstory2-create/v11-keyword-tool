...
[v20.0 변경 사항 - ContentFitScore 1단계(관찰용)]
  블로그 콘텐츠 적합성을 관찰하기 위해 IntentFit/Expandability/MonetizationFit
  3축으로 구성된 ContentFitScore를 계산해 결과 객체에 부가 필드로만 기록한다.
  final_score, adjusted_final_score, grade 판정, TOP5/TOP10 정렬, History
  Penalty, API 호출, 문서수 2버킷, DataLab 로직에는 전혀 관여하지 않는다.
  ENABLE_CONTENT_FIT_SCORE 플래그로 on/off 가능. 하드 필터(부적합 필터)에
  걸린 항목은 ContentFitScore를 계산하지 않고 None으로 남긴다.

[v19.9 변경 사항 - 블로그 작성 부적합 키워드 필터 1단계]
  1·2차 연관검색어 확장이 끝나 pool이 완성된 직후, 문서수 확인(3단계) 이전에
  블로그 콘텐츠로 부적합한 키워드(계산기류/고객센터류/실시간 단일 데이터류/
  불완전 어절)를 pool에서 분리한다. 완전 삭제하지 않고 held_inappropriate로
  보관했다가, 문서수·DataLab API 호출 및 TOP5/TOP10 후보 선정에서는 제외한
  채 최종 finalized에는 grade="보류"로 합류시켜 keyword_history.json에도
  기존과 동일하게 기록되도록 한다. ENABLE_INAPPROPRIATE_FILTER 플래그로
  on/off 가능.

[v19.8 변경 사항 - History Penalty 1단계(최근 TOP5/TOP10 반복 노출 완화)]
  keyword_history.json에 저장되어 온 데이터를 읽어 최근 7일 내 TOP5/TOP10
  등장 횟수(1회당 3% 감점, 최대 15%)를 adjusted_final_score에 반영한다.
  final_score 원본은 변경하지 않으며, 위험/보류 판정에는 영향을 주지 않는다.
  ENABLE_HISTORY_PENALTY 플래그로 on/off 가능. _update_keyword_history() 등
  기존 히스토리 저장 구조는 전혀 수정하지 않았다.

[v19.7.1 변경 사항 - keyword_history.json 경로 정리]
  KEYWORD_HISTORY_FILE을 실행 위치 기준 상대경로 대신 BASE_DIR 기준
  절대경로로 변경(app.py의 sys.frozen 분기 패턴과 동일). 점수 계산/등급
  분류/API 호출에는 영향 없음.
...
[출력 계약] score_candidates()는 (results, api_health) 튜플을 반환한다.
  - v20.0 신규 필드(관찰용): content_fit_score, content_intent_fit,
    content_expandability, content_monetization_fit,
    content_fit_matched_positive, content_fit_matched_negative
...
# ---- [v19.8 신규] History Penalty 1단계(최근 TOP5/TOP10 반복 노출 완화) ----
...
HISTORY_PENALTY_RATE_PER_HIT = 0.03  # 등장 1회당 감점률
HISTORY_PENALTY_MAX_RATE = 0.15      # 누적 감점률 상한

# ---- [v19.9 신규] 블로그 작성 부적합 키워드 필터 1단계 ----
ENABLE_INAPPROPRIATE_FILTER = True   # False면 필터 없이 기존과 100% 동일 동작

INAPPROPRIATE_CALCULATOR_SUFFIXES = ("계산기",)
INAPPROPRIATE_CENTER_SUFFIXES = ("고객센터", "콜센터", "상담센터")
INAPPROPRIATE_REALTIME_SUFFIXES = ("시세",)
INAPPROPRIATE_REALTIME_EXACT = ("환율",)
INAPPROPRIATE_INCOMPLETE_SUFFIXES = ("환급받", "환급하", "청약하", "신청하", "대출받")

# 아래 접미어로 끝나면 위 조건에 걸리더라도 필터링하지 않는다(오탐 방지, 우선 검사).
INAPPROPRIATE_WHITELIST_SUFFIXES = (
    "조회", "확인", "신청", "지급일", "환급일",
    "전화번호", "바로가기", "홈페이지", "로그인", "앱",
)

# ---- [v20 신규, 1단계: 관찰용] ContentFitScore (3축: Intent/Expandability/Monetization) ----
ENABLE_CONTENT_FIT_SCORE = True   # False면 계산/로그를 건너뛰고 필드에 None만 채움

CONTENT_FIT_WEIGHT_INTENT = 0.45
CONTENT_FIT_WEIGHT_MONETIZATION = 0.35
CONTENT_FIT_WEIGHT_EXPANDABILITY = 0.20

# IntentFit 사전 (긍정/부정 단일 리스트)
INTENT_FIT_POSITIVE = ("방법", "조건", "자격", "비교", "후기", "추천", "총정리", "차이", "장단점")
INTENT_FIT_NEGATIVE = ("계산기", "고객센터", "콜센터", "상담센터", "시세", "환율", "로그인", "바로가기")
INTENT_FIT_POSITIVE_BONUS = 2.0
INTENT_FIT_POSITIVE_MAX_HITS = 3
INTENT_FIT_NEGATIVE_PENALTY = 3.0

# MonetizationFit 사전 (전환 의도어 단일 리스트, category_score 재사용 없음)
MONETIZATION_POSITIVE = (
    "가입", "신청", "비교", "견적", "대출", "카드",
    "보험료", "금리", "환급", "청구", "갈아타기",
)
MONETIZATION_POSITIVE_BONUS = 2.0
MONETIZATION_POSITIVE_MAX_HITS = 3

# Expandability (사전 없음, 형제 키워드 수 + IntentFit 긍정 리스트 재활용)
EXPANDABILITY_CHILD_WEIGHT_RATIO = 0.6
EXPANDABILITY_DIVERSITY_WEIGHT_RATIO = 0.4
EXPANDABILITY_DIVERSITY_UNIT = 2.5

# 로그 보강용(최저 N건 나열)
CONTENT_FIT_LOG_LOWEST_N = 5
...
# =========================================================================
# 3C. [v19.9 신규] 블로그 작성 부적합 키워드 필터 1단계
# =========================================================================
def _check_inappropriate_keyword(keyword):
    """키워드 하나를 판정. (매칭여부, 사유문자열 또는 None)을 반환."""
    kw = (keyword or "").strip()
    if not kw:
        return False, None

    if kw.endswith(INAPPROPRIATE_WHITELIST_SUFFIXES):
        return False, None

    if kw.endswith(INAPPROPRIATE_CALCULATOR_SUFFIXES):
        return True, f"[부적합필터] 계산기형('{kw}')"

    if kw.endswith(INAPPROPRIATE_CENTER_SUFFIXES):
        return True, f"[부적합필터] 고객센터형('{kw}')"

    if kw.endswith(INAPPROPRIATE_REALTIME_SUFFIXES) or kw in INAPPROPRIATE_REALTIME_EXACT:
        return True, f"[부적합필터] 실시간단일데이터형('{kw}')"

    if kw.endswith(INAPPROPRIATE_INCOMPLETE_SUFFIXES):
        return True, f"[부적합필터] 불완전어절('{kw}')"

    return False, None


def _split_inappropriate_candidates(pool, log=None):
    """
    pool 완성 직후 호출한다. 부적합 키워드를 pool에서 분리해 held_inappropriate로
    반환한다. held_inappropriate 항목은 doc/DataLab API 호출 대상에서 제외되도록
    doc_count=None 등 held_v와 동일한 필드를 미리 채워둔다.
    """
    clean_pool = []
    held_inappropriate = []
    for c in pool:
        matched, reason = _check_inappropriate_keyword(c.get("keyword", ""))
        if not matched:
            clean_pool.append(c)
            continue
        ...
        entry["doc_api_failed"] = False
        entry["doc_check_skipped"] = False
        entry["doc_call_aborted"] = False
        entry["datalab_ratio"] = DATALAB_NEUTRAL_RATIO
        entry["datalab_failed"] = False
        entry["datalab_skipped"] = True
        entry["verify_datalab"] = False
        entry["_inappropriate_filtered"] = True
        entry["_inappropriate_reason"] = reason
        held_inappropriate.append(entry)

    if log:
        log(f"[scorer] 블로그 부적합 키워드 필터(1단계) 적용 - "
            f"보류 처리 {len(held_inappropriate)}건 / 검증 대상 유지 {len(clean_pool)}건")

    return clean_pool, held_inappropriate
...
# =========================================================================
# 8B. [v20 신규, 1단계: 관찰용] ContentFitScore
# =========================================================================
def _build_anchor_sibling_index(candidates):
    index = {}
    for c in candidates:
        anchor = c.get("anchor", "")
        index.setdefault(anchor, []).append(c.get("keyword", ""))
    return index


def _compute_intent_fit(keyword):
    kw = keyword or ""
    matched_pos = [w for w in INTENT_FIT_POSITIVE if w in kw][:INTENT_FIT_POSITIVE_MAX_HITS]
    matched_neg = [w for w in INTENT_FIT_NEGATIVE if w in kw]

    score = 5.0
    score += len(matched_pos) * INTENT_FIT_POSITIVE_BONUS
    score -= len(matched_neg) * INTENT_FIT_NEGATIVE_PENALTY

    return round(max(0.0, min(10.0, score)), 2), matched_pos, matched_neg


def _compute_monetization_fit(keyword):
    kw = keyword or ""
    matched_pos = [w for w in MONETIZATION_POSITIVE if w in kw][:MONETIZATION_POSITIVE_MAX_HITS]

    score = 5.0 + len(matched_pos) * MONETIZATION_POSITIVE_BONUS

    return round(max(0.0, min(10.0, score)), 2), matched_pos


def _compute_expandability(entry, anchor_sibling_index):
    anchor = entry.get("anchor", "")
    self_kw = entry.get("keyword", "")
    siblings = [kw for kw in anchor_sibling_index.get(anchor, []) if kw != self_kw]

    if not siblings:
        return 5.0  # 형제 없음(API 미시도/실패 포함) - 콘텐츠 결함과 구분하기 위한 중립값

    child_score = min(10.0, float(len(siblings)))

    distinct_types = set()
    for kw in siblings:
        for w in INTENT_FIT_POSITIVE:
            if w in kw:
                distinct_types.add(w)
    diversity_score = min(10.0, len(distinct_types) * EXPANDABILITY_DIVERSITY_UNIT)

    score = (
        EXPANDABILITY_CHILD_WEIGHT_RATIO * child_score
        + EXPANDABILITY_DIVERSITY_WEIGHT_RATIO * diversity_score
    )
    return round(max(0.0, min(10.0, score)), 2)


def _apply_content_fit_score(entry, anchor_sibling_index):
    """
    관찰용 필드만 채운다. final_score/adjusted_final_score/grade/정렬에는
    절대 관여하지 않는다. held_inappropriate(하드 필터 통과 항목)는 계산하지
    않고 None으로 남긴다.
    """
    if entry.get("_inappropriate_filtered"):
        entry["content_fit_score"] = None
        entry["content_intent_fit"] = None
        entry["content_expandability"] = None
        entry["content_monetization_fit"] = None
        entry["content_fit_matched_positive"] = []
        entry["content_fit_matched_negative"] = []
        return

    keyword = entry.get("keyword", "")
    intent_fit, intent_pos, intent_neg = _compute_intent_fit(keyword)
    monetization_fit, monetization_pos = _compute_monetization_fit(keyword)
    expandability = _compute_expandability(entry, anchor_sibling_index)

    content_fit_score = round(
        CONTENT_FIT_WEIGHT_INTENT * intent_fit
        + CONTENT_FIT_WEIGHT_MONETIZATION * monetization_fit
        + CONTENT_FIT_WEIGHT_EXPANDABILITY * expandability,
        2
    )

    entry["content_fit_score"] = content_fit_score
    entry["content_intent_fit"] = intent_fit
    entry["content_expandability"] = expandability
    entry["content_monetization_fit"] = monetization_fit
    entry["content_fit_matched_positive"] = intent_pos + monetization_pos
    entry["content_fit_matched_negative"] = intent_neg


# =========================================================================
# 9. keyword_history.json 저장/로드 + History Penalty 1단계
# =========================================================================
...
def _apply_history_penalty(entry, appearance_counts):
    """최근 TOP5/TOP10 등장 횟수 기반 adjusted_final_score 계산."""
...
def score_candidates(candidates, apis, log=None, max_workers=MAX_WORKERS):
    ...
    if ENABLE_INAPPROPRIATE_FILTER:
        pool, held_inappropriate = _split_inappropriate_candidates(pool, log)
    else:
        held_inappropriate = []
    ...
    all_candidates = datalab_targets + datalab_skip + doc_unknown + held_v + held_inappropriate

    if ENABLE_CONTENT_FIT_SCORE:
        anchor_sibling_index = _build_anchor_sibling_index(all_candidates)
    ...
    for entry in all_candidates:
        ...
        if ENABLE_CONTENT_FIT_SCORE:
            _apply_content_fit_score(entry, anchor_sibling_index)
        else:
            entry["content_fit_score"] = None
            entry["content_intent_fit"] = None
            entry["content_expandability"] = None
    ...
    for entry in scored:
        if entry["doc_count"] is None:
            ...
            if entry.get("_inappropriate_filtered"):
                entry["hold_reasons"] = [
                    entry.get("_inappropriate_reason", "[부적합필터] 부적합 키워드")
                ]
            elif entry.get("volume_invalid_keyword"):
                entry["hold_reasons"] = [
    ...
    for rank, entry in enumerate(finalized, start=1):
        ...
        entry.pop("_inappropriate_filtered", None)
        entry.pop("_inappropriate_reason", None)
    ...
    if log:
        ...
        if ENABLE_INAPPROPRIATE_FILTER:
            log(f"[scorer] 블로그 부적합 키워드 필터(1단계) 최종 보류 처리 {len(held_inappropriate)}건 "
                f"(계산기류/고객센터류/실시간단일데이터류/불완전어절)")
        ...
        if ENABLE_CONTENT_FIT_SCORE:
            fit_entries = [e for e in finalized if e.get("content_fit_score") is not None]
            skipped_fit_count = sum(1 for e in finalized if e.get("content_fit_score") is None)

            if fit_entries:
                fit_values = [e["content_fit_score"] for e in fit_entries]
                avg_fit = sum(fit_values) / len(fit_values)
                low_fit_count = sum(1 for v in fit_values if v < 4.0)
                log(f"[scorer] ContentFitScore(v20 1단계, 관찰용) 계산 {len(fit_entries)}건 - "
                    f"평균 {avg_fit:.2f}, 4.0 미만 {low_fit_count}건 / "
                    f"하드필터로 미계산 {skipped_fit_count}건 "
                    f"(final_score/등급/정렬에는 미반영)")

                top_grade_entries = [e for e in finalized if e.get("grade") in ("TOP5", "TOP10")]
                if top_grade_entries:
                    log("[scorer] ContentFitScore - TOP5/TOP10 키워드별 세부 점수:")
                    for e in top_grade_entries:
                        log(f"  · [{e.get('grade')}] '{e.get('keyword','')}' - "
                            f"종합 {e.get('content_fit_score')} "
                            f"(Intent {e.get('content_intent_fit')} / "
                            f"Monetization {e.get('content_monetization_fit')} / "
                            f"Expandability {e.get('content_expandability')})")

                lowest_entries = sorted(fit_entries, key=lambda e: e["content_fit_score"])[:CONTENT_FIT_LOG_LOWEST_N]
                log(f"[scorer] ContentFitScore - 최저 {len(lowest_entries)}건:")
                for e in lowest_entries:
                    log(f"  · [{e.get('grade')}] '{e.get('keyword','')}' - "
                        f"종합 {e.get('content_fit_score')} "
                        f"(Intent {e.get('content_intent_fit')} / "
                        f"Monetization {e.get('content_monetization_fit')} / "
                        f"Expandability {e.get('content_expandability')}) "
                        f"부정매칭={e.get('content_fit_matched_negative')}")
