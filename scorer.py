...
SEARCH_INTENT_SIGNALS = ["방법", "신청", "조건", "대상", "기준", "총정리", "확인", "자격",
                          "지원금", "환급", "세금", "할인", "가격", "비교", "기한", "서류"]
CTR_SIGNALS = ["논란", "반전", "결국", "충격", "사망", "폭행", "이혼", "열애",
               "파문", "폭로", "긴급", "발칵", "역대급", "속보", "단독"]
ACTION_SIGNALS = ["접수", "신청", "등록", "발급", "예약", "조회", "지급", "환급", "계산"]
EVENT_VERB_SIGNALS = ["시작", "마감", "확정", "발표", "인상", "인하", "확대", "축소", "연장", "종료", "개시", "중단"]
DETAIL_PATTERNS = [r"만?\s*\d+세\s*[~-]\s*\d+세", r"\d+년\s*\d+월\s*\d+일",
                    r"\d+월\s*(초|중|말)?까지", r"\d+%", r"\d+[,\d]*\s?만?\s?원"]

QUESTION_SPECS = [
    {"question": "{event}, 신청 대상은 누구일까?",
     "seo_title": "{event} 신청 대상 총정리, 나도 받을 수 있을까", "requires": "always"},
    {"question": "{event}, 신청 방법은 어떻게 될까?",
     "seo_title": "{event} 신청 방법 총정리 (절차·서류 안내)", "requires": "always"},
    {"question": "{event}, 신청 시 주의할 점은 무엇일까?",
     "seo_title": "{event} 신청 전 반드시 확인해야 할 주의사항", "requires": "always"},
    {"question": "{event}, 언제까지 확인해야 할까?",
     "seo_title": "{event} 신청 기한{detail_suffix}, 놓치면 못 받는다", "requires": "detail"},
    {"question": "{event}, {related}와는 어떤 관련이 있을까?",
     "seo_title": "{event}, {related} 함께 확인하면 좋은 정보", "requires": "related"},
]
CLICKBAIT_TEMPLATES = {
    "soft": ["{event}, 모르면 손해 보는 이유", "{event}, 놓치면 못 받는다 지금 확인",
             "{event}, 다들 헷갈리는 이 부분", "{event}, 신청 안 하면 후회하는 이유",
             "{event}, 이 조건 모르고 지나치면 손해"],
    "strong": ["{event}, 알고 보니 이런 반전이 있었다", "{event} 소식에 다들 이렇게 반응했다",
               "결국 터졌다... {event}의 실제 배경", "{event}, 이 사실 알고 계셨나요?",
               "{event} 논란, 어디까지 이어질까"],
}
HYBRID_TEMPLATES = {
    "soft": ["{event} 신청 대상은 누구일까? 놓치면 후회하는 이유", "{event} 신청 방법, 궁금하다면 지금 확인",
             "{event} 주의할 점, 모르면 손해 보는 이유", "{event} 신청 기한 임박, 지금 확인해야{detail_suffix}",
             "{event}, {related}까지 한번에 정리 (놓치지 마세요)"],
    "strong": ["{event} 신청 대상 논란, 나도 해당될까?", "{event} 신청 방법, 다들 이렇게 반응했다",
               "{event} 주의할 점, 알고 보니 반전 있었다", "{event} 마감 임박, 몰랐다면 충격{detail_suffix}",
               "{event}, {related} 관련 진실은 이것"],
}


def _tokenize(text):
    return [t for t in re.findall(r"[가-힣A-Za-z0-9]+", text) if len(t) > 1 and t not in STOPWORDS]


def extract_keywords(title, summary, top_n=6):
...
def build_core_summary(summary, max_sentences=3):
...
def extract_event(text, main_keyword, action_hits):
    components = []
    if main_keyword:
...
def generate_questions(event, related, has_detail):
    related_word = related[0] if related else None
    questions = []
    for spec in QUESTION_SPECS:
        req = spec["requires"]
        include = (req == "always" or (req == "detail" and has_detail) or (req == "related" and related_word))
        if include:
            questions.append(spec["question"].format(event=event, related=related_word or event))
    return questions


def build_outline(event, related, has_detail):
    outline = [f"{event}, 무엇이 달라지나 (개요)", "신청 대상 및 조건", "신청 방법 및 절차"]
    if has_detail:
...
def classify_strategy(search_hits, ctr_hits):
    ...
    return {"type": s_type, "reason": reason, "search_signals_found": search_hits, "ctr_signals_found": ctr_hits}


def build_seo_titles(event, related, detail):
    detail_suffix = f" ({detail} 기준)" if detail else ""
    related_word = related[0] if related else event
    return [spec["seo_title"].format(event=event, related=related_word, detail_suffix=detail_suffix)
            for spec in QUESTION_SPECS]


def build_clickbait_titles(event, tone):
    return [t.format(event=event) for t in CLICKBAIT_TEMPLATES[tone]]


def build_hybrid_titles(event, related, detail, tone):
    detail_suffix = f" ({detail} 기준)" if detail else ""
    related_word = related[0] if related else event
    return [t.format(event=event, related=related_word, detail_suffix=detail_suffix)
            for t in HYBRID_TEMPLATES[tone]]


def generate_titles(event, related, detail, search_hits, ctr_hits):
...
# GUI 호환 wrapper: log, progress 등 어떤 추가 인자가 와도 절대 에러 없이 흡수
def score_candidates(news_title, news_summary, *args, **kwargs):
...
if __name__ == "__main__":
    ...
    sample_summary = ("국토교통부는 오늘부터 청년 월세 지원금 신청 접수를 시작한다고 밝혔다. "
